...
Ebola-like gamma infection kernel (pre-intervention) $\Kpre(\tau)$ (mean: 16.2 days, CV: 0.58, and \Ro: 1.5) is shown in black \citep{park2019practical}.
...
A constant-strength intervention reduces the density by a constant proportion: $\Kpost(\tau) = \Kpre(\tau)/\theta$; when the strength of intervention matches the strength of epidemic ($\theta = \RR$), the resulting distribution is equivalent to the intrinsic generation-interval distribution ($\Kpost(\tau) = g(\tau)$).
(B) A constant-speed intervention with $\phi \approx 0.0267/\mathrm{day}$ is applied to the same kernel.
A constant-speed intervention reduces the density exponentially: $\Kpost(\tau) = \Kpre(\tau) \exp(-\phi \tau)$; when the speed of intervention matches the speed of epidemic ($\phi = r$), the resulting distribution is equivalent to the initial backward generation-interval distribution ($\Kpost(\tau) = b(\tau)$). 
}
\figlab{constant}
\end{figure}
...
\begin{equation}
	\Kpost(\tau) = \Kpre(\tau)\exp(-\phi\tau) = b(\tau)\exp((r-\phi)\tau)
\end{equation}
...
\begin{equation}
	1 = \int \Kpost(\tau) \exp(-\hat r\tau) d\tau.	
\end{equation}
...
\begin{equation}
K(\tau) = \RR \left(\tsub{p}{early} \tsub{f}{early}(\tau) + (1-\tsub{p}{early}) \tsub{f}{late}(\tau) \right).
\end{equation}
...
$\tsub{p}{early} = 0.23$. We assume that the initial speed of the epidemic is $\ro=0.452\,\mathrm{year}^{-1}$(\figref{example}B), and ask what value of $\Ro$ would produce this rate of growth.
...
Here we are estimating the ``basic'' reproductive number \Ro, using information about the initial rate of spread \ro, as a proxy for the difficulty of eventually eliminating HIV. 
...
The estimated strength of such an intervention is simply the average of $\tsub{L}{condom}$, i.e., $\theta=4$, whereas the estimated strength of the epidemic \Ro\ decreases as the proportion of early transmission \pEarly\ increases (\figref{condom}B).
...
(B) The estimated amount of early transmission affects estimated epidemic strength \Ro, but not intervention strength $\theta$ of a condom-based intervention.
...
(B) Increase in the estimated amount of early transmission decreases the estimated epidemic strength \RR\ as well as the estimated strength of a test-and-treat intervention $\theta$.
...
(D) The estimated amount of early transmission has little effect on the effective speed of intervention $\phi$, and none on the initial speed of the epidemic estimated from incidence data \rr.
