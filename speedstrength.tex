...
\caption{
\textbf{Impact of pre-symptomatic transmission on the infection kernel of COVID-19.}
...
In order to understand the impact of pre-symptomatic transmission on control of COVID-19, we assume that a newly infected individual go through latent, pre-symptomatic, and symptomatic stages for durations of $D_e$, $D_p$, and $D_s$ days, respectively.
...
\begin{equation}
\Kpre(\tau) = \beta_p \mathbb{P}(D_e \leq \tau < D_e + D_p) + \beta_s \mathbb{P}(D_e + D_s \leq \tau < D_e + D_p + D_s).
\end{equation}
Integrating across $\tau$, the pre-intervention reproduction number is given by:
\begin{equation}
\Rpre = \beta_p \mathbb{E}(D_p) + \beta_s \mathbb{E}(D_s).
\end{equation}
The pre-intervention generation-interval distribution is given by:
\begin{equation}
\gpre(\tau) = \frac{p \, \mathbb{P}(D_e \leq \tau < D_e + D_p)}{\mathbb{E}(D_p)} + \frac{(1-p) \, \mathbb{P}(D_e + D_s \leq \tau < D_e + D_p + D_s)}{\mathbb{E}(D_s)},
\end{equation}
where $p = \beta_p E(D_p)/\Rpre$ represents the amount of pre-symptomatic transmission.
Therefore, more early transmission (higher $p$), means shorter generation intervals and, for a given value of observed \rpre, less transmission per individual --- that is, a lower value of \Rpre (\figref{covid}).
...
More early transmission does not change our estimate of the pre-intervention speed of the epidemic \rpre, which is inferred from data. But it changes our estimate of the \emph{effective} speed of a given intervention because more early transmission gives more weight in the calculation of effective speed ($\phi$) to the early period of infection.

If control depends on identifying cases from symptoms, the post-intervention kernel can be modeled as:
\begin{equation}
\begin{aligned}
...
\end{aligned}
\end{equation}
where the hazard of control $h_s$ is likely to increase through time. 
...
It is also clear from \eqref{symptom} that as $p$ increases, the amount of transmission reduced by the control will decrease.
Conversely, if control is mostly done by tracing travellers and contacts, we can write
\begin{equation}
\Kpost(\tau) = \Kpre(\tau) \exp\left(-\int_0^\tau h_i(\sigma) d\sigma\right),
\eqlab{trace}
\end{equation}
where $h_i$ is likely to start high and decrease as cases are most likely to be identified early on. 
In this case more weight on the early hazard of control will mean a faster effective speed. 
Thus, control will be easier than originally thought.
Once again, it is clear from \eqref{trace} that we can control the spread as long of $h_i$ is sufficiently high (i.e., the speed of intervention is sufficiently fast), regardless of the amount of pre-symptomatic transmission $p$.
