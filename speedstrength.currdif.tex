...
%DIF DEL speedstrength.oct.tex   Sun Dec 27 21:59:47 2020
...
\DIFdelbegin \DIFdel{Since we are interested in invasion and control, we will generally neglect changes in $K(\tau, t)$ through time and assume $K(\tau, t) \equiv K(\tau)$.
...
Imagine \DIFaddbegin \DIFadd{we have a pre-intervention $\Kpre = \Rpre g(\tau)$ (we generally suppress the suffix in }\gpre\DIFadd{\ for readability) and }\DIFaddend a control measure that proportionally reduces $K$ by a factor of $\theta$, for example, by protecting a fixed fraction of susceptibles through vaccination (\figref{constant}A). We then have:
\begin{equation}
