...
%DIF DEL speedstrength.tex.cb8e114.oldfile   Sun Jan  3 12:28:30 2021
%DIF ADD speedstrength.tex                   Sun Jan  3 14:38:10 2021
...
The insight that \DIFdelbegin \DIFdel{a case }\DIFdelend \DIFaddbegin \DIFadd{an infection }\DIFaddend must on average cause at least one new \DIFdelbegin \DIFdel{case }\DIFdelend \DIFaddbegin \DIFadd{infection }\DIFaddend under good conditions for a disease to persist goes back $>100$ years \citep{ross1911prevention}; \DIFaddbegin \DIFadd{SWP: what do you mean good conditions...
}\DIFaddend the idea of averaging by defining a `typical' \DIFdelbegin \DIFdel{case }\DIFdelend \DIFaddbegin \DIFadd{infection }\DIFaddend was formalized 30 years ago \citep{diekmann1990definition}.
...
Ebola-like gamma infection kernel (pre-intervention) $\Kpre(\tau)$ (mean: 16.2 days, CV: 0.58, and \DIFdelbeginFL %DIFDELCMD < \Ro%%%
...
\begin{equation}
	\Kpost(\tau) = (\DIFdelbegin %DIFDELCMD < \RR%%%
...
\DIFdelend \DIFaddbegin \Rpost \DIFaddend = \int \Kpost(\tau) d\tau.
\end{equation}
...
\begin{equation}
	\Kpost(\tau) = \Kpre(\tau)\exp(-\phi\tau) = b(\tau)\exp((\DIFdelbegin \DIFdel{r-}\DIFdelend \DIFaddbegin \rpre\DIFadd{-}\DIFaddend \phi)\tau)
\end{equation}
...
\begin{equation}
	1 = \int \Kpost(\tau) \exp(-\DIFdelbegin %DIFDELCMD < \hat %%%
...
\begin{equation}
\DIFdelbegin \DIFdel{K}\DIFdelend \DIFaddbegin \Kpre\DIFaddend (\tau) = \DIFdelbegin %DIFDELCMD < \RR %%%
...
$\tsub{p}{early} = 0.23$. We assume that the \DIFdelbegin \DIFdel{initial }\DIFdelend \DIFaddbegin \DIFadd{pre-intervention }\DIFaddend speed of the epidemic is \DIFdelbegin \DIFdel{$\ro=0.452\,\mathrm{year}^{-1}$}\DIFdelend \DIFaddbegin \DIFadd{$\rpre=0.452\,\mathrm{year}^{-1}$}\DIFaddend (\figref{example}B), and ask what value of \DIFdelbegin \DIFdel{$\Ro$ }\DIFdelend \DIFaddbegin \DIFadd{$\Rpre$ }\DIFaddend would produce this rate of growth.
When transmission is fast, (i.e., when \pEarly\ is large), individuals don't need to transmit as much to achieve this speed, so the estimated value of \DIFdelbegin %DIFDELCMD < \Ro%%%
...
Here we are estimating the \DIFdelbegin \DIFdel{``basic'' reproductive number }%DIFDELCMD < \Ro%%%
\DIFdelend \DIFaddbegin \DIFadd{pre-intervention reproductive number }\Rpre\DIFaddend , using information about the initial rate of spread \DIFdelbegin %DIFDELCMD < \ro%%%
\DIFdelend \DIFaddbegin \rpre\DIFaddend , as a proxy for the difficulty of eventually eliminating HIV. 
...
The estimated strength of such an intervention is simply the average of $\tsub{L}{condom}$, i.e., $\theta=4$, whereas the estimated strength of the epidemic \DIFdelbegin %DIFDELCMD < \Ro%%%
...
(B) The estimated amount of early transmission affects estimated epidemic strength \DIFdelbeginFL %DIFDELCMD < \Ro%%%
\DIFdelendFL \DIFaddbeginFL \Rpre\DIFaddendFL , but not intervention strength $\theta$ of a condom-based intervention.
...
\includegraphics[width=\textwidth]{code/coronaexample.pdf}
\caption{
\textbf{\DIFaddFL{Impact of pre-symptomatic transmission on the infection kernel of COVID-19.}}
...
(B) Corresponding pre-intervention reproduction numbers are calculated assuming a pre-intervention doubling time of 3 days (i.e., $r = \log(2)/3\,\textrm{days}^{-1}$).
}}
\figlab{covid}
\end{figure}

\DIFaddend A generation-interval perspective \cite{park2019practical} can be used to challenge this view.
\DIFdelbegin \DIFdel{More early transmission }\DIFdelend \DIFaddbegin \DIFadd{In order to understand the impact of pre-symptomatic transmission on control of COVID-19, we assume that a newly infected individual go through latent, pre-symptomatic, and symptomatic stages for durations of $D_e$}\DIFaddend , \DIFdelbegin \DIFdel{for a given value of observed }%DIFDELCMD < \rpre%%%
\DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{$D_p$, and $D_s$ days, respectively.
During pre-symptomatic and symptomatic stages, they transmit infection at rate $\beta_p$ and $\beta_s$, respectively.
In this case, the pre-intervention infection kernel can be written as:
}\begin{equation}
\DIFadd{\Kpre(\tau) = \beta_p \mathbb{P}(D_e \leq \tau < D_e + D_p) + \beta_s \mathbb{P}(D_e + D_s \leq \tau < D_e + D_p + D_s).
}\end{equation}
\DIFadd{Integrating across $\tau$, the pre-intervention reproduction number is given by:
}\begin{equation}
\DIFadd{\Rpre = \beta_p \mathbb{E}(D_p) + \beta_s \mathbb{E}(D_s).
}\end{equation}
\DIFadd{The pre-intervention generation-interval distribution is given by:
}\begin{equation}
\DIFadd{\gpre(\tau) = \frac{p \, \mathbb{P}(D_e \leq \tau < D_e + D_p)}{\mathbb{E}(D_p)} + \frac{(1-p) \, \mathbb{P}(D_e + D_s \leq \tau < D_e + D_p + D_s)}{\mathbb{E}(D_s)},
}\end{equation}
\DIFadd{where $p = \beta_p E(D_p)/\Rpre$ represents the amount of pre-symptomatic transmission.
Therefore, more early transmission (higher $p$), }\DIFaddend means shorter generation intervals and\DIFdelbegin \DIFdel{therefore }\DIFdelend \DIFaddbegin \DIFadd{, for a given value of observed }\rpre\DIFadd{, }\DIFaddend less transmission per individual --- that is, a lower value of \Rpre \DIFaddbegin \DIFadd{(\figref{covid})}\DIFaddend .
...
More early transmission does not change our estimate of the \DIFdelbegin \DIFdel{initial }\DIFdelend \DIFaddbegin \DIFadd{pre-intervention }\DIFaddend speed of the epidemic \rpre, which is inferred from data. But it \DIFdelbegin \DIFdel{does change }\DIFdelend \DIFaddbegin \DIFadd{changes }\DIFaddend our estimate of the \emph{effective} speed of a given \DIFdelbegin \DIFdel{control curve. 
...
If control depends on identifying cases from symptoms, \DIFdelbegin \DIFdel{we might expect the }\DIFdelend \DIFaddbegin \DIFadd{the post-intervention kernel can be modeled as:
}\begin{equation}
\DIFadd{\begin{aligned}
\Kpost(\tau) &= \frac{p \Rpre \, \mathbb{P}(D_e \leq \tau < D_e + D_p)}{\mathbb{E}(D_p)}\\
&+ \left(\frac{(1-p) \Rpre \, \mathbb{P}(D_e + D_s \leq \tau < D_e + D_p + D_s)}{\mathbb{E}(D_s)}\right) \exp\left(-\int_0^\tau h_s(\sigma) d\sigma\right),
\eqlab{symptom}
\end{aligned}
}\end{equation}
\DIFadd{where the }\DIFaddend hazard of control \DIFaddbegin \DIFadd{$h_s$ is likely }\DIFaddend to increase through time. 
In this case, more early transmission will give more weight to the low effectiveness of control early in infection: thus, the effective speed of control will be lower than originally thought, and control will be harder.
\DIFaddbegin \DIFadd{It is also clear from \eqref{symptom} that as $p$ increases, the amount of transmission reduced by the control will decrease.
}\DIFaddend Conversely, if control is mostly done by tracing travellers and contacts, \DIFdelbegin \DIFdel{cases might be }\DIFdelend \DIFaddbegin \DIFadd{we can write
}\begin{equation}
\DIFadd{\Kpost(\tau) = \Kpre(\tau) \exp\left(-\int_0^\tau h_i(\sigma) d\sigma\right),
\eqlab{trace}
}\end{equation}
\DIFadd{where $h_i$ is likely to start high and decrease as cases are }\DIFaddend most likely to be identified early on\DIFdelbegin \DIFdel{: in }\DIFdelend \DIFaddbegin \DIFadd{. 
In }\DIFaddend this case more weight on the early hazard of control will mean a faster effective speed. 
Thus, control will be easier than originally thought\DIFaddbegin \DIFadd{.
Once again, it is clear from \eqref{trace} that we can control the spread as long of $h_i$ is sufficiently high (i.e., the speed of intervention is sufficiently fast), regardless of the amount of pre-symptomatic transmission $p$}\DIFaddend .
