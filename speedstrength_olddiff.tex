...
%DIF ADD speedstrength.tex                       Sun Dec 27 21:54:21 2020
...
Ebola-like gamma infection kernel \DIFdelbeginFL \DIFdelFL{$K(\tau)$ }\DIFdelendFL (\DIFaddbeginFL \DIFaddFL{pre-intervention) $\Kpre(\tau)$ (}\DIFaddendFL mean: 16.2 days, CV: 0.58, and \Ro: 1.5) is shown in black \citep{park2019practical}.
...
A constant-strength intervention reduces the density by a constant proportion: \DIFdelbeginFL \DIFdelFL{$\hat K(\tau) = K(\tau)/\theta$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\Kpost(\tau) = \Kpre(\tau)/\theta$}\DIFaddendFL ; when the strength of intervention matches the strength of epidemic (\DIFdelbeginFL \DIFdelFL{$\theta = \mathcal R$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\theta = \RR$}\DIFaddendFL ), the resulting distribution is equivalent to the intrinsic generation-interval distribution (\DIFdelbeginFL \DIFdelFL{$\hat K(\tau) = g(\tau)$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\Kpost(\tau) = g(\tau)$}\DIFaddendFL ).
(B) A constant-speed intervention with $\phi \approx 0.0267/\mathrm{day}$ is applied to the same kernel.
A constant-speed intervention reduces the density exponentially: \DIFdelbeginFL \DIFdelFL{$\hat K(\tau) = K(\tau) \exp(-\phi \tau)$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\Kpost(\tau) = \Kpre(\tau) \exp(-\phi \tau)$}\DIFaddendFL ; when the speed of intervention matches the speed of epidemic ($\phi = r$), the resulting distribution is equivalent to the initial backward generation-interval distribution (\DIFdelbeginFL \DIFdelFL{$\hat K(\tau) = b(\tau)$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\Kpost(\tau) = b(\tau)$}\DIFaddendFL ). 
}
\figlab{constant}
\end{figure}
...
\DIFdel{K}\DIFdelend \DIFaddbegin \Kpost\DIFaddend (\tau) = (\RR/\theta) g(\tau).
\end{equation}
...
\begin{equation}
	\Rhat = \int \DIFdelbegin %DIFDELCMD < \hat %%%
...
$\tsub{p}{early} = 0.23$. We assume that the initial speed of the epidemic is \DIFdelbegin \DIFdel{$r=0.452\,\mathrm{year}^{-1}$}\DIFdelend \DIFaddbegin \DIFadd{$\ro=0.452\,\mathrm{year}^{-1}$}\DIFaddend (\figref{example}B), and ask what value of \DIFdelbegin \DIFdel{$\RR$ }\DIFdelend \DIFaddbegin \DIFadd{$\Ro$ }\DIFaddend would produce this rate of growth.
...
\DIFdelend \DIFaddbegin \Ro\DIFaddend \ decreases (\figref{example}C).
Therefore, as \pEarly\ gets smaller, we expect stronger intervention to be required in order to control the disease.
\DIFaddbegin \DIFadd{Here we are estimating the ``basic'' reproductive number }\Ro\DIFadd{, using information about the initial rate of spread }\ro\DIFadd{, as a proxy for the difficulty of eventually eliminating HIV. 
}\DIFaddend 
...
\DIFdelend \DIFaddbegin \Ro\DIFaddend \ decreases as the proportion of early transmission \pEarly\ increases (\figref{condom}B).
...
\DIFdelendFL \DIFaddbeginFL \Ro\DIFaddendFL , but not intervention strength \DIFdelbeginFL %DIFDELCMD < \Rhat %%%
...
(B) Increase in the estimated amount of early transmission decreases the estimated epidemic strength \RR\ as well as the estimated strength of a test-and-treat intervention \DIFdelbeginFL %DIFDELCMD < \Rhat%%%
