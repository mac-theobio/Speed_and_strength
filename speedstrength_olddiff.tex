...
%DIF ADD speedstrength.tex                   Sun Jan 31 17:57:43 2021
...
More early transmission \DIFaddbegin \DIFadd{means shorter generation intervals and}\DIFaddend , for a given value of observed \rpre, \DIFdelbegin \DIFdel{means shorter generation intervals and therefore }\DIFdelend less transmission per individual --- that is, a lower value of \Rpre.
...
More early transmission does not change our estimate of the \DIFdelbegin \DIFdel{initial }\DIFdelend \DIFaddbegin \DIFadd{pre-intervention }\DIFaddend speed of the epidemic \rpre, which is inferred from data. 
But it \DIFdelbegin \DIFdel{does change }\DIFdelend \DIFaddbegin \DIFadd{changes }\DIFaddend our estimate of the \emph{effective} speed of a given \DIFdelbegin \DIFdel{control curve. 
...
Thus, control will be easier than originally thought}\DIFdelend \DIFaddbegin \DIFadd{on---in this case, more early transmission causes the speed of intervention to increase.
}

\DIFadd{In \figref{covid}, we provide a concrete example using a compartmental model, which assumes that infected individuals progress sequentially through three stages of infection: exposed, pre-symptomatic, and symptomatic (see Supplementary Materials for details).
...
\DIFadd{Here, we see that increasing the proportion of pre-symptomatic transmission $p$ shortens the generation time (\figref{covid}A) and reduces the pre-intervention epidemic strength (\figref{covid}B). But it does not change the pre-intervention epidemic speed (\figref{covid}C).
\figref{covid}B--C further confirm that the proportion of pre-symptomatic transmission $p$ has opposite effects on the effectiveness of symptom-based intervention---modeled as a constant hazard applied to individuals in symptomatic compartments---and infection-basde intervention---modeled in a similar way as the test-and-treat intervention for HIV}\DIFaddend .
